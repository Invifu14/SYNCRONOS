...
const { calcularCartaAstral } = require('./astrology');
const app = express();
const PORT = 3000;
...
(async () => {
    ...
    try { await db.run("ALTER TABLE usuarios ADD COLUMN hora_nacimiento TEXT;"); } catch(e) { /* Ya existe, ignorar */ }
    try { await db.run("ALTER TABLE usuarios ADD COLUMN lugar_nacimiento TEXT;"); } catch(e) { /* Ya existe, ignorar */ }
    try { await db.run("ALTER TABLE usuarios ADD COLUMN luna TEXT;"); } catch(e) { /* Ya existe, ignorar */ }
    try { await db.run("ALTER TABLE usuarios ADD COLUMN ascendente TEXT;"); } catch(e) { /* Ya existe, ignorar */ }
    try { await db.run("ALTER TABLE usuarios ADD COLUMN venus TEXT;"); } catch(e) { /* Ya existe, ignorar */ }
    try { await db.run("ALTER TABLE usuarios ADD COLUMN marte TEXT;"); } catch(e) { /* Ya existe, ignorar */ }
...
app.post('/registrar-cronos', async (req, res) => {
    ...
    const { nombre, fecha_nacimiento, hora_nacimiento, lugar_nacimiento, ubicacion, gustos, metodo_registro, correo, telefono, intencion, genero, genero_interes, latitud, longitud } = req.body;
    const generacion = obtenerGeneracion(fecha_nacimiento);
    const { signo, foto } = obtenerSignoYFoto(fecha_nacimiento);

    // Calcular carta astral avanzada si existe hora de nacimiento
    let luna = "Desconocido", ascendente = "Desconocido", venus = "Desconocido", marte = "Desconocido";
    if (fecha_nacimiento && hora_nacimiento) {
        const carta = calcularCartaAstral(fecha_nacimiento, hora_nacimiento, latitud, longitud);
        luna = carta.luna;
    ...
    if (existing) {
        // Actualizamos los campos nuevos por si es un usuario viejo haciendo login de nuevo
        if (hora_nacimiento) {
             await db.run(
                 `UPDATE usuarios SET hora_nacimiento = ?, lugar_nacimiento = ?, luna = ?, ascendente = ?, venus = ?, marte = ? WHERE nombre = ?`,
                 [hora_nacimiento, lugar_nacimiento, luna, ascendente, venus, marte, nombre]
             );
        }
        const updatedUser = await db.get(`SELECT * FROM usuarios WHERE nombre = ?`, [nombre]);
        return res.json({ mensaje: "Login OK", usuario: updatedUser });
    }

    await db.run(
        `INSERT INTO usuarios (nombre, fecha_nacimiento, generacion, signo_zodiacal, foto, ubicacion, gustos, metodo_registro, correo, telefono, intencion, genero, genero_interes, latitud, longitud, hora_nacimiento, lugar_nacimiento, luna, ascendente, venus, marte) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [nombre, fecha_nacimiento, generacion, signo, foto, ubicacion || "", gustos || "", metodo_registro || "", correo || "", telefono || "", intencion || "", genero || "", genero_interes || "", latitud || null, longitud || null, hora_nacimiento || "", lugar_nacimiento || "", luna, ascendente, venus, marte]
    );
